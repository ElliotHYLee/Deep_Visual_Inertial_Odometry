...
%     kfseqRMSE(seq+1,:) = getRMSEN(posKF, gt_pos, N, seq, cutN)
    ...
    seqRMSE(seq+1,:) = getDriftN(pr_dtr, gt_dtr, cutN)
    kfseqRMSE(seq+1,:) = getDriftN(velKF, gt_dtr, cutN)
end


for i =1:1:length(gt_pos)
   gt_dist(i) = norm(gt_pos(i,:));
   pr_dist(i) = norm(pr_pos(i,:));    
end
