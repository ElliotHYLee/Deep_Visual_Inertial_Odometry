from MyPyTorchAPI.CNNUtils import *
import numpy as np
from MyPyTorchAPI.CustomActivation import *
from SE3Layer import GetTrans
from LSTMFC import LSTMFC
from LSTM import LSTM
from MyPyTorchAPI.MatOp import *
from Model_RNN_KF import Model_RNN_KF
from Model_RNN_General import Model_RNN_General
class Model_RNN_Ensemble(nn.Module):
    def __init__(self, dsName='airsim', delay=10):
        super().__init__()

        self.kf = Model_RNN_KF()
        self.gen = Model_RNN_General()

        self.fc0 = nn.Sequential(nn.Linear(3, 64), nn.PReLU(),
                                 nn.Linear(64, 64), nn.PReLU(),
                                 nn.Linear(64, 64), nn.PReLU(),
                                 nn.Linear(64, 3), Sigmoid(0.01, 1))

        self.fc1 = nn.Sequential(nn.Linear(3, 64), nn.PReLU(),
                                 nn.Linear(64, 64), nn.PReLU(),
                                 nn.Linear(64, 64), nn.PReLU(),
                                 nn.Linear(64, 3), Sigmoid(1, 1))

    def forward(self, dt, acc, acc_stand, pr_dtr_gnd, dtr_cv_gnd, gt_dtr_gnd_init, sysCovInit=None):
        velKF, accCov, sysCov = self.kf.forward(dt, acc, acc_stand, pr_dtr_gnd, dtr_cv_gnd, gt_dtr_gnd_init, sysCovInit=None)
        velGen = self.gen.forward(dt, acc, acc_stand, pr_dtr_gnd, dtr_cv_gnd, gt_dtr_gnd_init)
        w = self.fc0(velGen)
        velEns = torch.mul(w, velGen) + torch.mul(1-w, velKF)

        return velEns, accCov, sysCov, velKF

if __name__ == '__main__':
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    delay = 10
    m = nn.DataParallel(Model_RNN_General(delay=delay)).to(device)
    dt = torch.ones((2, delay, 1), dtype=torch.float).cuda()*0.1
    acc = torch.ones((2, delay, 3), dtype=torch.float).cuda()
    acc_stand = torch.ones((2, delay, 3), dtype=torch.float).cuda()
    pr_dtr_gnd = torch.zeros((2, delay, 3), dtype=torch.float).cuda()
    dtr_cv_gnd = torch.zeros((2, delay, 3, 3), dtype=torch.float).cuda()
    gt_dtr_gnd_init = torch.zeros((2, 3), dtype=torch.float).cuda()

    # a = acc.cumsum(1)
    # print(a)
    # print(a.shape)
    acc_cov, corr_vel = m.forward(dt, acc, acc_stand, pr_dtr_gnd, dtr_cv_gnd, gt_dtr_gnd_init)
    # print(acc_cov)
    # print(acc_cov.shape)
    print(corr_vel)
    print(corr_vel.shape)
